  ...
  const { error } = withDb((db) => {
    ...
    if (templateIds && templateIds.length > 0) {
      for (const id of templateIds) {
        db.run(
          'INSERT OR IGNORE INTO room_templates (room_id, template_id) VALUES (?, ?)',
          [roomId, id],
        );
      }
    }
...
export function dbCreateTemplate(
  name: string,
  role: string,
  persona?: string,
  capabilities?: string,
...
export function dbUpdateAgentPersona(
  name: string,
  persona: string,
): { error?: string } {
  const { error } = withDb((db) => {
...
export function dbUpdateAgentCapabilities(
  name: string,
  capabilities: string[],
): { error?: string } {
  const { error } = withDb((db) => {
    db.run('UPDATE agents SET capabilities = ? WHERE name = ?', [
...
export function dbRemoveAgentFromRoom(
  agentName: string,
  room: string,
): { error?: string } {
  const { error } = withDb((db) => {
...
export function dbDeleteAgent(name: string): {
  removed_from_rooms: string[];
  error?: string;
} {
...
export function dbCreateRoomTemplate(
  name: string,
  topic: string | null,
  agentTemplateIds: number[],
): { error?: string } {
  const { error } = withDb((db) => {
    db.run(
...
export function dbUpdateRoomTemplate(
  id: number,
  field: 'name' | 'topic' | 'agent_template_ids',
  value: string,
): { error?: string } {
  const allowed = ['name', 'topic', 'agent_template_ids'];
  if (!allowed.includes(field)) return { error: 'Invalid field' };
  const { error } = withDb((db) => {
    db.run(`UPDATE room_template_definitions SET ${field} = ? WHERE id = ?`, [
      value,
      id,
    ]);
  });
...
export function dbSetRoomTemplates(
  room: string,
  templateIds: number[],
): { error?: string } {
  const { error } = withDb((db) => {
...
export function dbClearAgentPane(
  name: string,
  pane: string,
): { error?: string } {
  const { error } = withDb((db) => {
    db.run(
      "UPDATE agents SET pane = NULL, status = 'unknown' WHERE name = ? AND pane = ?",
      [name, pane],
    );
  });
  return error ? { error } : {};
