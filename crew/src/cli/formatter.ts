...
const FORMATTERS: Record<string, (data: any) => string> = {
  ...
  'send-batch': (d) => {
    const workers = d.workers
      .map(
        (w: any) =>
          `  ${w.name}: ${w.dispatch_status}${w.ctx_pct != null ? ` context-window:${w.ctx_pct}%${w.ctx_pct >= 80 ? ' ⚠ compact/clear encouraged' : ''}` : ''}`,
      )
      .join('\n');
  ...
  goal: (d) => {
    if (d.error) return `Error: ${d.error}`;
    if (d.goal_status)
      return `Goal ${d.goal_status}${d.message ? ` — ${d.message}` : ''}${d.ctx_pct != null ? ` context-window:${d.ctx_pct}%${d.ctx_pct >= 80 ? ' ⚠ compact/clear encouraged' : ''}` : ''}`;
    if (d.removed)
      return `${d.message ?? 'Goal removed'}${d.ctx_pct != null ? ` context-window:${d.ctx_pct}%${d.ctx_pct >= 80 ? ' ⚠ compact/clear encouraged' : ''}` : ''}`;
    ...
    if (d.overview) {
      ...
      for (const g of d.goals) {
        ...
        lines.push(
          `${mark} ${g.agent_name}: "${g.description}" (${g.status}, turn ${g.turn_count ?? 0})${g.ctx_pct != null ? ` context-window:${g.ctx_pct}%${g.ctx_pct >= 80 ? ' ⚠ compact/clear encouraged' : ''}` : ''}`,
        );
      }
      return lines.join('\n');
    }
    ...
    if (d.goal)
      return `🎯 ${d.goal.agent_name}: "${d.goal.description}" (${d.goal.status}, turn ${d.goal.turn_count ?? 0})${d.redone_from ? ` — redone from #${d.redone_from}` : ''}${d.goal.ctx_pct != null ? ` context-window:${d.goal.ctx_pct}%${d.goal.ctx_pct >= 80 ? ' ⚠ compact/clear encouraged' : ''}` : ''}`;
