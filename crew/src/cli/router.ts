...
export const COMMANDS: Record<string, { handler: Handler; buildParams: ParamBuilder }> = {
  ...
  send:           { handler: handleSendMessage, buildParams: (f) => ({ room: f.room, text: f.text, name: f.name, to: f.to, mode: f.mode ?? 'push', kind: f.kind ?? 'chat', reply_to: f['reply-to'] != null ? parseInt(String(f['reply-to']), 10) : undefined, sender_pane: f['sender-pane'] }) },
