...
export default function TemplateModal({ template, onClose, onSuccess }: Props) {
  ...
  const [role, setRole] = useState<AgentTemplate['role']>(template?.role ?? 'worker');
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const body = { name: name.trim(), role, persona: persona || undefined, capabilities: capabilities || undefined };
      if (template) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-slate-800 rounded-lg p-6 w-96 space-y-4" onClick={e => e.stopPropagation()}>
        <h2 className="text-slate-100 font-semibold">{template ? 'Edit Template' : 'New Template'}</h2>
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs text-slate-400 uppercase tracking-widest">Role</label>
            <select
              value={role}
          ...
          <div>
            <label className="text-xs text-slate-400 uppercase tracking-widest">Persona</label>
            <textarea
              value={persona}
          ...
          <div>
            <label className="text-xs text-slate-400 uppercase tracking-widest">Capabilities</label>
            <textarea
              value={capabilities}
