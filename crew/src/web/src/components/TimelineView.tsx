...
export default function TimelineView() {
  ...
  const toPct = (ms: number) =>
    Math.max(0, Math.min(100, ((ms - windowStart) / windowMs) * 100));
    ...
    <div className="flex-1 flex flex-col overflow-hidden min-w-0">
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-slate-700 flex-shrink-0 flex-wrap">
        ...
        <div className="flex items-center gap-1 ml-2">
          <button
            onClick={zoomOut}
            className="px-2 py-0.5 text-xs rounded text-slate-400 hover:text-slate-200 font-mono"
          >
            −
          </button>
      ...
      <div className="flex-1 overflow-auto" ref={containerRef}>
        <div style={{ minWidth: `${zoom * 100}%` }} className="flex flex-col">
          ...
          {rows.map((row) => (
            <div
              key={row.agentName}
              className="flex items-center border-b border-slate-800 h-8 hover:bg-slate-800/30"
            >
              {/* Label */}
              <div
                className="w-28 flex-shrink-0 px-2 text-xs text-slate-400 truncate"
                title={row.agentName}
              >
                {row.agentName}
              </div>
      ...
      {tooltip && (
        ...
        >
          <div className="font-medium text-slate-200">
            #{tooltip.seg.taskId} · {tooltip.seg.status}
          </div>
          <div className="text-slate-400 mt-0.5 truncate">
            {tooltip.seg.summary}
          </div>
          <div className="text-slate-600 mt-1">
            {new Date(tooltip.seg.startMs).toLocaleTimeString()} →{' '}
            {new Date(tooltip.seg.endMs).toLocaleTimeString()}
          </div>
        </div>
      )}
    </div>
