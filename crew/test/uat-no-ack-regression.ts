 ...
 */
import { initDb, closeDb, addAgent, addMessage, readMessages, getAllMessages } from '../src/state/index.ts';
...
{
  const msg = addMessage('worker-1', 'leader-1', 'main', 'task: build login page', 'pull', 'worker-1', 'task');
...
{
  addMessage('worker-1', 'leader-1', 'main', 'ping: are you alive?', 'pull', 'worker-1', 'chat');
  const { messages } = readMessages('worker-1');
  assert(messages.length >= 1, 'TC-2: worker can read messages without sending ACK',
    `found ${messages.length} messages`);
  assert(messages.some(m => m.text === 'ping: are you alive?'), 'TC-2: correct message content accessible');
}

// TC-3: Multiple messages accumulate in DB without ACK
{
  for (let i = 1; i <= 5; i++) {
    addMessage('worker-1', 'leader-1', 'main', `batch-msg-${i}`, 'pull', 'worker-1', 'chat');
  }
  const all = getAllMessages();
  const workerMessages = all.filter(m => m.to === 'worker-1');
  assert(workerMessages.length >= 5, 'TC-3: all 5 messages stored without ACK',
...
// TC-4: Cursor-based read works as polling fallback (simulates agent polling)
{
  ...
  addMessage('worker-2', 'leader-1', 'alpha', 'first task', 'pull', 'worker-2', 'task');
  addMessage('worker-2', 'leader-1', 'alpha', 'second task', 'pull', 'worker-2', 'task');

  const poll1 = readMessages('worker-2', 'alpha');
  assert(poll1.messages.length === 2, 'TC-4: first poll gets all pending messages',
    `got ${poll1.messages.length}`);

  // Second poll with same cursor — no new messages (idempotent)
  const { messages: newMsgs } = readMessages('worker-2', 'alpha', poll1.next_sequence);
  assert(newMsgs.length === 0, 'TC-4: second poll (same cursor) returns no new messages');

  // New message arrives, third poll picks it up
  addMessage('worker-2', 'leader-1', 'alpha', 'third task', 'pull', 'worker-2', 'task');
  const { messages: poll3 } = readMessages('worker-2', 'alpha', poll1.next_sequence);
  assert(poll3.length === 1, 'TC-4: third poll finds newly added message');
  assert(poll3[0]!.text === 'third task', 'TC-4: correct message content in third poll');
}
...
// TC-5: getPollingInterval returns 500ms in conservative mode
{
  ...
  assert(interval === 500, 'TC-5: stale heartbeat (35s) triggers conservative 500ms fallback',
    `got ${interval}ms`);
...
// TC-6: getPollingInterval returns role-based intervals for fresh agents (reduced profile)
{
  ...
  assert(workerInterval === 2_000, `TC-6: worker interval = 2000ms (got ${workerInterval}ms)`);
  assert(leaderInterval === 5_000, `TC-6: leader interval = 5000ms (got ${leaderInterval}ms)`);
  assert(bossInterval   === 10_000, `TC-6: boss interval = 10000ms (got ${bossInterval}ms)`);
}
...
// TC-7: getPollingInterval with no lastActivityMs uses profile only (no stale check)
{
  ...
  assert(interval === 2_000, `TC-7: no heartbeat data → uses profile interval (got ${interval}ms)`);
}
...
// TC-8: getPollingInterval stale threshold is exactly 30s boundary
{
  ...
  assert(staleInterval === 500,   `TC-8: 30001ms old → conservative 500ms (got ${staleInterval}ms)`);
  assert(freshInterval === 2_000, `TC-8: 29999ms old → reduced 2000ms  (got ${freshInterval}ms)`);
}
...
// TC-9: System works correctly when delivery mode is 'pull' (no push/ACK)
{
  ...
  const msg = addMessage('worker-3', 'boss-1', 'beta', 'no-ack task', 'pull', 'worker-3', 'task');
...
// TC-10: Multiple agents each poll independently — no cross-contamination
{
  ...
  addMessage('worker-4', 'leader-1', 'gamma', 'for worker-4', 'pull', 'worker-4', 'chat');
  addMessage('worker-5', 'leader-1', 'gamma', 'for worker-5', 'pull', 'worker-5', 'chat');

  const w4 = readMessages('worker-4');
  const w5 = readMessages('worker-5');

  assert(w4.messages.every(m => m.to === 'worker-4'), 'TC-10: worker-4 only sees its own messages');
  assert(w5.messages.every(m => m.to === 'worker-5'), 'TC-10: worker-5 only sees its own messages');
  assert(w4.messages.some(m => m.text === 'for worker-4'), 'TC-10: worker-4 receives correct content');
  assert(w5.messages.some(m => m.text === 'for worker-5'), 'TC-10: worker-5 receives correct content');
}
