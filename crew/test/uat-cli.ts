...
await run('crew rooms (empty)', ['rooms'], (o) => o.includes('(no rooms)'));
...
await run(
  'crew read',
  ['read', '--name', 'uat-bot', '--room', 'uat-test'],
...
await run('crew rooms --json', ['rooms', '--json'], (o) => {
  try {
    ...
    return (
      Array.isArray(d.rooms) && d.rooms.some((r: any) => r.name === 'uat-test')
    );
  } catch {
    return false;
  }
