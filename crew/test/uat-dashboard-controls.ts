...
mock.module('../src/tmux/index.ts', () => ({
  ...
  sendEscape: async (t: string) => {
    tmuxState.escapeCalls.push(t);
    return { delivered: true };
  },
  sendKeys: async (_t: string, k: string) => {
    tmuxState.keyCalls.push(k);
    return { delivered: true };
  },
...
const { revokeAgent, interruptAgent, clearAgentSession } = await import(
  '../src/dashboard/actions/agent-actions.ts'
);
const { interruptTask, cancelTask, reassignTask } = await import(
  '../src/dashboard/actions/task-actions.ts'
);
...
async function assertThrows(
  ...
  try {
    ...
    if (containing && !msg.toLowerCase().includes(containing.toLowerCase())) {
      console.error(
        `  ✗ ${label} — error "${msg}" doesn't contain "${containing}"`,
      );
      failed++;
    } else {
      console.log(`  ✓ ${label}`);
...
// TC-A1: revokeAgent — happy path
{
  ...
  assert(
    result.includes('Revoked') && result.includes('wk-01'),
    'returns "Revoked wk-01"',
    result,
  );
  // cleanupDeadAgentTasks overwrites 'interrupted' → 'error' for all remaining tasks
  assert(
    getTask(task.id)?.status === 'error',
    `task ended as error (got: ${getTask(task.id)?.status})`,
  );
...
// TC-A3: interruptAgent — happy path
{
  ...
  assert(
    result.includes('Interrupted') && result.includes('wk-02'),
    'returns "Interrupted wk-02"',
    result,
  );
  assert(
    result.includes(String(task.id)),
    `result contains task id #${task.id}`,
    result,
  );
  assert(
    getTask(task.id)?.status === 'interrupted',
    `task is interrupted (got: ${getTask(task.id)?.status})`,
  );
...
// TC-A4: interruptAgent — no active task
{
  ...
  await assertThrows(
    () => interruptAgent('wk-03'),
    'throws "no active task"',
    'no active task',
  );
  console.log();
}
...
// TC-A6: clearAgentSession — happy path
{
  ...
  assert(
    result.includes('Cleared') && result.includes('wk-05'),
    'returns "Cleared wk-05 session"',
    result,
  );
  assert(
    tmuxState.keyCalls.length === 2,
    `sendKeys called twice (got ${tmuxState.keyCalls.length})`,
  );
  assert(
    tmuxState.keyCalls[0] === '/clear',
    `first sendKeys is "/clear" (got "${tmuxState.keyCalls[0]}")`,
  );
  assert(
...
// TC-T1: interruptTask — happy path
{
  ...
  assert(
    result.includes(`Interrupted task #${task.id}`),
    'returns "Interrupted task #<id>"',
    result,
  );
  assert(
    getTask(task.id)?.status === 'interrupted',
    'task status is interrupted',
  );
  console.log();
}
...
// TC-T3: cancelTask — happy path
{
  ...
  assert(
    result.includes(`Cancelled task #${task.id}`),
    'returns "Cancelled task #<id>"',
    result,
  );
...
// TC-T5: reassignTask — from active
{
  ...
  assert(
    getTask(newId)?.summary === 'new task text',
    'new task has correct text',
  );
  assert(
    tmuxState.keyCalls.some((k) => k === 'new task text'),
    'sendKeys called with new text',
  );
  console.log();
}
...
// TC-T6: reassignTask — from queued
{
  ...
  assert(newId > task.id, `new task created (id #${newId})`);
  assert(
    getTask(newId)?.summary === 'revised text',
    'new task has correct text',
  );
  console.log();
}
