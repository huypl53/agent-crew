...
describe('hook goal reminder race conditions', () => {
  ...
  test.serial(
    'Stop reminder callback does not send when goal is done before delay elapses',
    async () => {
      const room = getOrCreateRoom(
        '/tmp/goal-reminder-race',
        'goal-reminder-race',
      );
      const pane = '%42';
      ...
      const reminders = stepLog.filter(
        (entry) => entry.op === 'sendKeys' && entry.target === pane,
      );
