...
class GoalReminderUATHarness {
  ...
  async startCatInPane(key: PaneKey): Promise<void> {
    await sendToPane(this.pane(key), 'exec cat');
    await Bun.sleep(300);
    this.trace.add(`${key} switched to cat`);
  }
  ...
  async assertNoPaneText(
    key: PaneKey,
    marker: string,
    text: string,
    settleMs = 1200,
  ) {
    await assertPaneAfterMarkerLacks(this.pane(key), marker, text, {
      settleMs,
    });
  }

  async waitForPaneText(
    key: PaneKey,
    marker: string,
    text: string,
    timeoutMs = 7000,
  ) {
    return waitForPaneAfterMarkerToContain(this.pane(key), marker, text, {
      timeoutMs,
    });
  }
  ...
  async waitForReminderOnStop(
    ...
    const result = await waitForPaneOutput(
      this.pane(key),
      pattern,
      7000,
      async () => {
...
describe('goal reminder UAT harness', () => {
  ...
  test.serial(
    'uat: worker reminder still appears on every Stop hook',
    async () => {
      ...
      await harness.startCatInPane('worker-1');
  ...
  test.serial(
    'uat: active worker goal suppresses leader completion until goal is done',
    async () => {
      const trace = new ScenarioTrace();
      const room = getRoom('crew');
      expect(room).toBeDefined();
      ...
      await harness.startCatInPane('worker-1');
      await harness.startCatInPane('leader');
      ...
      expect(firstReminder.matched).toBe(true);
      trace.add(
        'worker reminder observed while leader completion stayed gated',
      );

      await harness.assertNoPaneText(
        'leader',
        leaderMarker,
      ...
      trace.add(
        'confirmed leader did not receive Stop completion while goal was active',
      );
  ...
  test.serial(
    'uat: leader reminder appears only on Stop after queued direct delivery drains',
    async () => {
      const trace = new ScenarioTrace();
      const room = getRoom('crew');
      expect(room).toBeDefined();
      ...
      await deliverMessage('worker-1', 'crew', 'plain completion', 'lead-1');
  ...
  test.serial(
    'uat: blocked leader delays delivery and reminder arming until unblock flush',
    async () => {
      const trace = new ScenarioTrace();
      const room = getRoom('crew');
      expect(room).toBeDefined();
      ...
      await deliverMessage('worker-1', 'crew', 'blocked completion', 'lead-1');
      ...
      const unblockResult = await handleInputBlock({
        subcommand: 'off',
        name: 'lead-1',
      });
      trace.add(
        `leader unblocked: ${JSON.stringify(unblockResult.content[0])}`,
      );

      await harness.waitForPaneText(
        'leader',
        marker,
        'blocked completion',
        5000,
  ...
  test.serial(
    'uat: leader reminder is one-shot after direct delivery',
    async () => {
      const trace = new ScenarioTrace();
      const room = getRoom('crew');
      expect(room).toBeDefined();
      ...
      await deliverMessage('worker-1', 'crew', 'one-shot completion', 'lead-1');
      ...
      await processHookEventInput(
        JSON.stringify({
          hook_event_name: 'Stop',
          session_id: 'leader-stop-one-shot-2',
        }),
        harness.pane('leader'),
      );
      ...
      await harness.assertNoPaneText(
        'leader',
        marker,
        '🎯 Goal: One-shot direct reminder (turn 2)',
        1800,
      );
  ...
  test.serial(
    'uat: leader Stop during partial batch completion does not remind early',
    async () => {
      const trace = new ScenarioTrace();
      const room = getRoom('crew');
      expect(room).toBeDefined();
      ...
      await processHookEventInput(
        JSON.stringify({
          hook_event_name: 'Stop',
          session_id: 'leader-stop-partial-batch-1',
        }),
        harness.pane('leader'),
      );
  ...
  test.serial(
    'uat: batch final delivery arms leader reminder only after final summary drain',
    async () => {
      const trace = new ScenarioTrace();
      const room = getRoom('crew');
      expect(room).toBeDefined();
      ...
      expect(partialGoal?.leader_reminder_armed).toBe(0);
      trace.add(
        'confirmed no batch final output or leader arm after partial completion',
      );
      ...
      const finalOutput = await harness.waitForPaneText(
        'leader',
        marker,
        '🎯 Goal: Review batch results (turn 1)',
      );
