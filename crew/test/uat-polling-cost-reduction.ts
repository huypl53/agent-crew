...
{
  const r = await handleCheckChanges({
    name: 'lead-01',
    scopes: ['messages', 'tasks', 'agents'],
  });
  const s = scopes(r);
  assert(!r.isError, 'TC-V1: result is ok (no error)');
  assert(
    'messages' in s && 'tasks' in s && 'agents' in s,
    'TC-V1: all three scopes present',
  );
  assert(
    typeof s.messages!.version === 'number',
    'TC-V1: messages has numeric version',
  );
  assert(
    typeof s.messages!.updated_at === 'string',
    'TC-V1: messages has updated_at string',
  );
}
...
// TC-V2 — messages version bumps on new message
{
  ...
  assert(
    s2.messages!.version > mv1,
    'TC-V2: messages version bumped',
    `${mv1} → ${s2.messages!.version}`,
  );
...
// TC-V3 — tasks version bumps on task create
{
  ...
  assert(
    s2.tasks!.version > tv1,
    'TC-V3: tasks version bumped on create',
    `${tv1} → ${s2.tasks!.version}`,
  );
...
// TC-V4 — tasks version bumps on task status update
{
  ...
  const s1 = scopes(
    await handleCheckChanges({ name: 'lead-01', scopes: ['tasks'] }),
  );
  ...
  assert(
    s2.tasks!.version > tv1,
    'TC-V4: tasks version bumped on status update',
    `${tv1} → ${s2.tasks!.version}`,
  );
}
...
{
  const s1 = scopes(
    await handleCheckChanges({ name: 'lead-01', scopes: ['agents'] }),
  );
  ...
  assert(
    s2.agents!.version > av1,
    'TC-V5: agents version bumped on join',
    `${av1} → ${s2.agents!.version}`,
  );
}
...
// TC-V8 — default scopes (omit scopes param)
{
  ...
  assert(
    'messages' in s && 'tasks' in s && 'agents' in s,
    'TC-V8: all scopes returned by default',
  );
}
...
// TC-C1 — check_changes output is smaller than read_messages (≥10x)
{
  ...
  console.log(
    `  TC-C1: check_changes=${checkSz}B  read_messages=${readSz}B  ratio=${ratio}x`,
  );
  assert(
    checkSz * 10 <= readSz,
    'TC-C1: check_changes ≥10x smaller than read_messages',
    `ratio=${ratio}x`,
  );
...
// TC-C2 — check_changes output is smaller than get_status (≥5x)
{
  ...
  console.log(
    `  TC-C2: check_changes=${checkSz}B  get_status=${statusSz}B  ratio=${ratio}x`,
  );
  assert(
    checkSz * 5 <= statusSz,
    'TC-C2: check_changes ≥5x smaller than get_status',
    `ratio=${ratio}x`,
  );
...
// TC-C3 — stable versions when no state changes
{
  ...
  assert(
    s1.messages!.version === s2.messages!.version,
    'TC-C3: messages version stable',
  );
  assert(
    s1.tasks!.version === s2.tasks!.version,
    'TC-C3: tasks version stable',
  );
  assert(
    s1.agents!.version === s2.agents!.version,
    'TC-C3: agents version stable',
  );
