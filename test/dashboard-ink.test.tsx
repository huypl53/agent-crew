...
import type { Agent, Room, Message, Task } from '../src/shared/types.ts';
...
describe('DetailsPanel', () => {
  test('shows agent details with role and status', () => {
    ...
    const { lastFrame } = render(
      <DetailsPanel agent={agent} agentStatus={status} selectedNode={{ type: 'agent', id: 'agent:lead-1', label: 'lead-1' }} rooms={{}} messages={[]} tasks={[]} isSyncing={false} height={10} />
    );
  ...
  test('shows pane output', () => {
    ...
    const { lastFrame } = render(
      <DetailsPanel agent={agent} agentStatus={status} selectedNode={{ type: 'agent', id: 'agent:lead-1', label: 'lead-1' }} rooms={{}} messages={[]} tasks={[]} isSyncing={false} height={15} />
    );
  ...
  test('shows task summary when room selected', () => {
    ...
    const { lastFrame } = render(
      <DetailsPanel agent={null} agentStatus={null} selectedNode={{ type: 'room', id: 'room:fe', label: 'fe', memberCount: 2 }} rooms={rooms} messages={[]} tasks={tasks} isSyncing={false} height={12} />
    );
    const frame = lastFrame()!;
  ...
  test('shows syncing state', () => {
    const { lastFrame } = render(
      <DetailsPanel agent={null} agentStatus={null} selectedNode={null} rooms={{}} messages={[]} tasks={[]} isSyncing={true} height={10} />
    );
